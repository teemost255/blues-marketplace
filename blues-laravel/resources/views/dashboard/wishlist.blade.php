...
@if($wishlists->isEmpty())
    <div class="text-center py-20 text-slate-500">
        ...
        <a href="{{ route('dashboard.marketplace') }}" class="mt-4 inline-flex items-center gap-2 bg-brand hover:bg-brand-dark text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition-colors">Browse Marketplace</a>
    </div>
@else
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden hover:border-brand/50 transition-all group flex flex-col">
            ...
            <div class="p-4 flex flex-col flex-1">
                ...
                @endif
                <h3 class="font-semibold text-white text-sm mt-1 line-clamp-2">{{ $item->listing->title }}</h3>
                <div class="mt-auto pt-3 border-t border-slate-700 flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('dashboard.marketplace') }}" class="bg-brand hover:bg-brand-dark text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors">Buy</a>
                    </div>
                </div>
            </div>
        </div>
