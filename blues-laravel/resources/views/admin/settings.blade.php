...
<form method="POST" action="{{ route('admin.settings.update') }}" class="space-y-6 max-w-2xl">
    ...
    {{-- Virtual Numbers — HeroSMS (Server 1) --}}
    <div class="bg-slate-800 border border-slate-700 rounded-xl p-6">
        <div class="flex items-center gap-3 mb-5">
            ...
            <div>
                <h2 class="font-semibold text-white">Virtual Numbers — HeroSMS <span class="ml-2 text-xs font-bold bg-purple-700/60 text-purple-300 px-2 py-0.5 rounded-full">Server 1</span></h2>
                <p class="text-xs text-slate-400">API credentials for HeroSMS virtual number provisioning. Prices in USD, auto-converted to NGN.</p>
            </div>
        </div>
        <div class="space-y-4">
            <div>
            ...
            <div class="flex items-center justify-between bg-slate-700/50 border border-slate-600 rounded-lg px-4 py-3">
                <div>
                    <p class="text-sm font-medium text-white">HeroSMS Wallet</p>
                    <p class="text-xs text-slate-400 mt-0.5">Opens the HeroSMS deposit page in a new tab.</p>
                </div>
                <a href="https://hero-sms.com/balance" target="_blank" rel="noopener noreferrer"
                   class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    Top Up Wallet
                </a>
            </div>
