@php
    ...
    // Service icon: first letter of title, or category icon, as a coloured circle
    ...
    $colourIdx = abs(crc32($listing->title ?? '')) % count($colours);
    $iconBg   = $colours[$colourIdx];
...
<div class="bg-slate-800 border border-slate-700/50 rounded-2xl p-4 hover:border-brand/30 transition-all">
    ...
    {{-- ── Title row: icon + name + delivery badge ─────────────────────────── --}}
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
                <a href="{{ route('dashboard.marketplace.show', $listing->id) }}"
                    class="font-bold text-white text-sm leading-snug hover:text-brand transition-colors line-clamp-1">{{ $listing->title }}</a>
                ...
                @if($catLabel)
                <span class="shrink-0 text-[10px] font-semibold text-slate-400 border border-slate-600/50 rounded-full px-2 py-0.5 whitespace-nowrap">
                    {{ $catLabel }}
                </span>
                @endif
            </div>
    ...
    <div class="flex rounded-full overflow-hidden border border-brand/30 text-sm font-bold">
        ...
        @if($listing->stock > 0)
            ...
            <form method="POST" action="{{ route('dashboard.marketplace.buy', $listing->id) }}"
                ...
                @csrf
                <button type="submit"
                    class="w-full h-full flex items-center justify-center py-2.5 bg-slate-900 hover:bg-slate-950 text-white transition-all">
                    Buy
                </button>
            </form>
            @else
            <a href="{{ route('login') }}"
                class="flex-1 flex items-center justify-center py-2.5 bg-slate-900 hover:bg-slate-950 text-white transition-all">
                Buy
            </a>
            @endauth
        @else
            <div class="flex-1 flex items-center justify-center py-2.5 bg-slate-900 text-slate-600 cursor-not-allowed">
                Sold Out
            </div>
        @endif
    </div>
