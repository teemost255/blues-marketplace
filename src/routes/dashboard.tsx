...
import { Anchor, Bell, HelpCircle, LayoutDashboard, LogOut, Menu, Settings, ShoppingBag, Sparkles, User as UserIcon, Wallet } from "lucide-react";
  ...
  const SidebarContent = ({ onNav }: { onNav?: () => void }) => (
    <>
      <div className="p-5">
        ...
        <div className="mt-1 text-xs text-sidebar-foreground/60">{role === "admin" ? "Admin · " : ""}{user.email}</div>
      </div>
      <nav className="flex-1 space-y-1 px-3 overflow-y-auto">
        ...
        {role === "admin" && (
          <Link to="/admin" onClick={onNav} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-sidebar-accent/60">
      ...
      <div className="flex flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-2 border-b bg-background/80 px-4 backdrop-blur md:hidden">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="icon" asChild aria-label="Wallet"><Link to="/dashboard"><Wallet className="h-4 w-4" /></Link></Button>
          </div>
        </header>
