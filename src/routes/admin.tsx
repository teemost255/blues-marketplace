  ...
  const SidebarContent = ({ onNav }: { onNav?: () => void }) => (
    <>
      <div className="p-5">
        ...
        <div className="mt-3 text-xs text-red-700">Admin · {user?.email}</div>
      </div>
      ...
      <div className="flex min-h-screen bg-red-50">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-2 border-b border-red-200 bg-white/95 px-4 text-red-900 backdrop-blur md:hidden">
            ...
            <div className="text-sm text-red-700">{role}</div>
          </header>
              ...
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-red-700">
                    ...
                    <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">{role}</span>
                  </div>
                </div>
              </div>
            </div>
            {isModerator && (
              <div className="border-b border-red-200 bg-red-100 px-4 py-3 text-sm text-red-900 md:px-8">
                Moderator accounts have limited write access. Some user, listing, and ticket actions are disabled for your role.
              </div>
            )}
            <Outlet />
          </main>
